# This code run by Conda KAN in Broka
# KAN Mixer is used for Credit-g(CG) dataset
...
class KANMixerLayer(nn.Module):
    def __init__(
        ...
        super(KANMixerLayer, self).__init__()
        ...
        self.mixer_linear = nn.Linear(out_features, mixer_features)
        self.mixer_activation = nn.GELU()
        self.mixer_output = nn.Linear(mixer_features, out_features)

    def forward(self, x):
        x = self.linear(x)
        y = self.mixer_linear(x)
        y = self.mixer_activation(y)
        y = self.mixer_output(y)
        return x + y
...
class KAN_Mixer(nn.Module):
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        return sum(
            layer.linear.regularization_loss(regularize_activation, regularize_entropy)
            for layer in self.layers
        )
...
model = KAN_Mixer(kan_layers, mixer_features=64, grid_size=10, spline_order=3, scale_noise=0.01, scale_base=1.0, scale_spline=1.0, base_activation=torch.nn.SiLU, grid_eps=0.02, grid_range=[-1, 1])
