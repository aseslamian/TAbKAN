...
def freeze_all_except_last_cheby(model):
    # Identify all FractionalJacobiNeuralBlock layers dynamically
    cheby_indices = [i for i, layer in enumerate(model.layers) if layer.__class__.__name__ == "FractionalJacobiNeuralBlock"]
    linear_indices = [i for i, layer in enumerate(model.layers) if isinstance(layer, torch.nn.Linear)]
    
    if not cheby_indices:
        raise ValueError("No FractionalJacobiNeuralBlock found in model.")
    if not linear_indices:
        raise ValueError("No Linear layer found in model.")

    last_cheby_idx = max(cheby_indices)
    last_linear_idx = max(linear_indices)

    # Freeze all layers except the last FractionalJacobiNeuralBlock and last Linear layer
    for idx, layer in enumerate(model.layers):
        if idx in {last_cheby_idx, last_linear_idx}:  # Keep these layers trainable
