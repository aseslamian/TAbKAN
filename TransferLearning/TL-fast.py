...
def prepare_model(input_dim, output_dim, best_params):
    ...
    return FastKAN(width).to(device)

###########################################################################################
def freeze_all_except_last_cheby(model):
    # Find indices of all FastKANLayer instances
    cheby_indices = [i for i, layer in enumerate(model.layers) if 'FastKANLayer' in str(type(layer))]
    
    if not cheby_indices:
        raise ValueError("No FastKANLayer found in model.")
