...
def freeze_all_except_last_cheby(model):
    ...
    # Unfreeze last Symbolic_KANLayer
    if hasattr(model, 'symbolic_fun') and len(model.symbolic_fun) > 0:
        last_symbolic = model.symbolic_fun[-1]
        for param in last_symbolic.parameters():
...
def prepare_model(input_dim, output_dim, best_params):
    ...
    return KAN(width=width, grid=grid, k=k, device=device).to(device)
