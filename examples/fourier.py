...
print("\n--- [Step 2] Hyperparameter Tuning for FourierKAN ---")
...
search_space = {
    ...
    "gridsizes_layer_0": {"type": "int", "low": 4, "high": 10}, # Changed from 'orders'
    "gridsizes_layer_1": {"type": "int", "low": 4, "high": 10}, # Changed from 'orders'
    "lr": {"type": "float", "low": 1e-2, "high": 1.0, "log": True},
    "steps": {"type": "categorical", "choices": [10]}
}

# Create a dummy model instance to run the tuner
best_params = FourierKAN.tune(
    model_class=FourierKAN, # Pass the class itself
...
gridsizes = [best_params[f'gridsizes_layer_{i}'] for i in range(depth)]
pretrained_model = FourierKAN(layers=layers, gridsizes=gridsizes).to(device)
...
plt.title("Feature Importance from Pre-trained FourierKAN")
...
scratch_model = FourierKAN(layers=layers, gridsizes=gridsizes).to(device)
scratch_model.fit(
    target_dataset, 
    steps=steps, 
    loss_fn=nn.CrossEntropyLoss(), 
