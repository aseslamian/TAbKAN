...
import io # Needed for the deepcopy_model helper

# Assuming 'tabkan' is installed and in the Python path
from tabkan import FractionalKAN # Change the import

# --- 0. Setup and Configuration ---
device = 'cuda' if torch.cuda.is_available() else 'cpu'
print(f"--- Running TabKAN Comprehensive Example for fKAN on {device.upper()} ---")
...
def deepcopy_model(model):
    ...
    # This relies on the model class saving its config (e.g., self.layers_config)
    ...
    # Ensure the pretrained flag is carried over
...
# --- 2. Hyperparameter Tuning for FractionalKAN ---
print("\n--- [Step 2] Hyperparameter Tuning for FractionalKAN ---")
# fKAN uses 'orders' per layer, similar to ChebyshevKAN
search_space = {
...
# Run the tuner for FractionalKAN
best_params = FractionalKAN.tune(
    model_class=FractionalKAN,
...
pretrained_model = FractionalKAN(layers=layers, orders=orders).to(device)
...
try:
    ...
    plt.title("Feature Importance from Pre-trained FractionalKAN")
    plt.xlabel("Feature Index")
    plt.ylabel("Importance (L1 Norm of Weights)")
    plt.show()
except (AttributeError, NotImplementedError) as e:
    print(f"Could not get feature importance for FractionalKAN: {e}")
...
scratch_model = FractionalKAN(layers=layers, orders=orders).to(device)
scratch_model.fit(
    target_dataset,
    steps=steps,
    loss_fn=nn.CrossEntropyLoss(),
