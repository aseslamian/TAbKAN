...
def deepcopy_model(model):
    ...
    This is more robust than copy.deepcopy().
    """
    # Create a new instance of the model's class with the same arguments
    new_model = model.__class__(
        width=model.model.width,  # Accessing the underlying pykan model's config
        grid=model.model.grid,
        k=model.model.k,
        device=model.model.device
    )

    # Save the state_dict of the original model to an in-memory buffer
    buffer = io.BytesIO()
    ...
    # Make sure the new model knows it has been pretrained
...
print("\n--- [Step 2] Hyperparameter Tuning for SplineKAN ---")

# Define a search space for Optuna
# Note: For a real run, n_trials should be higher (e.g., 50-100)
search_space = {
    "depth": {"type": "int", "low": 1, "high": 2},
    "neurons_layer_0": {"type": "int", "low": 4, "high": 8},
    "neurons_layer_1": {"type": "int", "low": 4, "high": 8},
    "grid": {"type": "int", "low": 3, "high": 5},              # New hyperparameter
    "k": {"type": "int", "low": 1, "high": 3},                  # New hyperparameter (spline order, often fixed at 3)
    "lr": {"type": "float", "low": 1e-2, "high": 1.0, "log": True},
    "steps": {"type": "categorical", "choices": [1]}
}

# Create a dummy model instance to run the tuner
best_params = SplineKAN.tune(
    model_class=SplineKAN, # Pass the class itself
...
grid = best_params.pop("grid") # Get the grid size
k = best_params.pop("k")       # Get the spline order

# The 'width' parameter for SplineKAN is the same as the 'layers' list
layers = [source_dim] + [best_params[f'neurons_layer_{i}'] for i in range(depth)] + [2]

# Instantiate SplineKAN with its specific arguments
pretrained_model = SplineKAN(width=layers, grid=grid, k=k, device=device)

# Pre-train the model (this will call the original pykan's trainer)
...
plt.title("Feature Importance from Pre-trained SplineKAN")
...
scratch_model = SplineKAN(width=layers, grid=grid, k=k, device=device)
scratch_model.fit(
    target_dataset, 
    steps=steps, 
    loss_fn=nn.CrossEntropyLoss(), 
